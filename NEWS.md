...
- Fix `hmap()` crash when ComplexHeatmap expects `gpar()` (row/column name gp now always valid).
- Improve top-annotation color mapping: missing levels are filled with `grey80` with a warning instead of erroring.
- Add optional control of row/column name font size and font face via `*_names_gp`.
...
- pcafun: fixed shape handling (shapekey = metadata column; optional shape_map).
- prep_go_exp: robust GO export without hard-coded renaming; safe with missing databases.
- hmap: aligns metadata to sample order and checks ComplexHeatmap dependency.
